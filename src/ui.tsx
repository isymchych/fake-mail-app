...
export function buildApp$(state: State) {
  const {
    selectedFolder$,
    letters$,
    visibleLetters$,
  ...
  const lettersUI$ = combineLatest([selectedLetterId$, currentDate$])
    .pipe(
      mergeMap(([selectedLetterId, now]) =>
        visibleLetters$.pipe(
          map((letters) => letters.map((letter) => (
            ...
            >
              ...
              <div className="letter-date">
                {formatRelative(letter.date, now)}
              </div>
            </li>
          ))),
        )
  ...
  const selectedLetterUI$ = combineLatest([selectedLetter$, currentDate$])
    .pipe(
      map(([selectedLetter, now]) => {
        if (!selectedLetter) {
          return null;
        }
          ...
          <>
            ...
            <article>
              ...
              <div className="letter-preview-date">
                <span className="label">DATE:</span> {formatRelative(selectedLetter.date, now)}
              </div>
